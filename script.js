    ...
    $.getJSON("https://earthquake.usgs.gov/earthquakes/feed/v1.0/summary/4.5_week.geojson", function (data) {
        ...
        if (document.body.classList.contains("portada")) {
            data.features.forEach(function (temblor, i) {
                var articuloTipo;
                if (temblor.properties.place.includes("reece")) {
                    articuloTipo = '<article class="local">';
                } else {
                    articuloTipo = '<article class="global">';
                }
                $("section").append(articuloTipo + "<h2>" + temblor.properties.mag + "</h2><p>" + temblor.properties.place + "</p><p></p><h5>" + new Date(temblor.properties.time).toLocaleTimeString('en-CL', {day: 'numeric', month: 'numeric', year: 'numeric' ,  hour: 'numeric', minute: 'numeric' })+ "</p><p><a href='page.html?temblor=" + i + "'>ver mapa</a></p></article>");
            });
            var valor
